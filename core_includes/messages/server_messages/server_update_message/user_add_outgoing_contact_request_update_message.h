#ifndef USER_ADD_OUTGOING_CONTACT_REQUEST_UPDATE_MESSAGE_H
#define USER_ADD_OUTGOING_CONTACT_REQUEST_UPDATE_MESSAGE_H
...
private:
    QString contactName;
public:
    User_Add_Outgoing_Contact_Request_Update_Message(QString contactName) {this->contactName = contactName;}
    virtual QDataStream& writeToStream(QDataStream& stream) const
    {
        Server_Update_Message::writeToStream(stream);
        stream << Server_Update_Message::Server_Update_Message_Type::User_Add_Outgoing_Contact_Request_Update_Message << contactName;
        return stream;
    }
    static User_Add_Outgoing_Contact_Request_Update_Message* readMessageFromStream(QDataStream& stream)
    {
        QString contactName;
        stream >> contactName;
        return new User_Add_Outgoing_Contact_Request_Update_Message(contactName);
    }
