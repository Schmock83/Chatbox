    ...
    , socket(new QTcpSocket(this))
{
	...
	//TODO remove ----
	//add 4 default contacts
	for (int i = 0; i < 4; i++) {
		QPushButton* push_btn = new QPushButton(QString::number(i + 1));
		ui->chat_list_layout->addWidget(push_btn);
		chatButtons.push_back(push_btn);
	}

	QPushButton* btn = new QPushButton("0");
	addTopChatButton(btn);
	updateChatList();
	//TODO remove ----
...
void MainWindow::updateChatList()
{
	...
	QLayoutItem* wItem;
	while ((wItem = ui->chat_list_layout->layout()->takeAt(0)) != nullptr)
		delete wItem;

	//re-add them
	for (const auto& button : chatButtons)
		ui->chat_list_layout->addWidget(button);
}
